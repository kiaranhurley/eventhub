...
| 2026-05-12 | Kubernetes manifests from Compose | Cursor / Claude | Generate `k8s/` from `docker-compose.yml`: replicas (web 2, booking 2, others 1), NodePort 30080, ConfigMap for broker/Redis/booking URL, `/healthz` liveness | Added `kustomization.yaml` for apply order; initContainers for startup deps (later **`redis:7-alpine`** instead of busybox for reliable pulls on Desktop); `command: ["python","main.py"]` to skip wait-for-it in-cluster; small threaded **HTTP /healthz on port 9100** in payment/ticket/notification so probes match ARCHITECTURE | Good. | Compose `depends_on` becomes initContainers + ClusterIP DNS; pure AMQP workers need an explicit health endpoint for kube probes. |
| 2026-05-13 | K8s init `ImagePullBackOff` on Docker Desktop | Cursor (diagnosis + patch) | User pods stuck in `Init:ImagePullBackOff` while Redis/RabbitMQ ran; traced init images to `busybox:1.36` | Replaced all busybox init images with **`redis:7-alpine`** (`redis-cli` / `nc` / `wget`), `imagePullPolicy: IfNotPresent`; README note | Good. | Reuse images the cluster already pulled; a tiny utility image from Hub can fail independently of your app images. |
| 2026-05-13 | `ErrImageNeverPull` vs `docker images` on Docker Desktop | Cursor (kubectl + docs) | Ran `kubectl get pods/events`, `kubectl run … Never`, `kubectl get nodes`; Compose tags exist in Docker but kubelet rejected `eventhub-main-*` | README rewrite: **kind** (`desktop-control-plane`) vs image store; point to **containerd image store** toggle or **kubeadm** provisioner; add `imgcheck` one-liner | Good. | Docker CLI and Kubernetes do not always share one image store; Docker Desktop’s matrix (kind vs kubeadm × containerd vs Docker store) explains “image exists” yet `ErrImageNeverPull`. |
| 2026-05-13 | “Run commands until K8s fixed” from agent | Cursor | Tried `docker desktop kubernetes reset-cluster`, compose rebuild, local `registry:2` + push via port-forward and NodePort; `UseContainerdSnapshotter` already true in settings-store | Reverted failed in-cluster registry experiment; added `scripts/k8s-apply-and-check.ps1`; README note that **Docker Desktop GUI** (provisioner / containerd) cannot be driven from repo scripts | Mixed. | Host registry + `host.docker.internal` hits **registry-mirror 500** on kind; NodePort push to localhost timed out; some fixes are still manual UI on Windows. |
| 2026-05-13 | Kubeadm provisioner for local images | Human (Docker Desktop) + Cursor | Switched Kubernetes from **kind** to **kubeadm**; asked agent to re-apply stack | Human changed provisioner in Docker Desktop; Cursor ran `docker compose build`, `kubectl apply -k k8s/`, waited for pods — all **Running** on node **`docker-desktop`** | Good. | **kubeadm** shares the Docker image store with Compose builds; **`Never`** + `eventhub-main-*` works once the node name is `docker-desktop`, not `desktop-control-plane`. |
| 2026-05-13 | Pytest suite for rubric | Cursor | Add booking unit tests (mock Redis/pika paths), unit tests for payment/ticket/notification, contract/integration/e2e/performance/security; run `pytest -m unit` | Hand-written booking tests in **`booking_service_tests/`** (avoid nested `tests/` name clash with repo `tests`); other services loaded via `importlib.util`; integration/e2e/perf **skip** if Redis/booking URL unreachable; bandit on `web-frontend` | Good. | Pytest + unique module names avoids `main` collisions; markers separate fast unit vs optional live checks. |
| 2026-05-13 | Final test wording + push | Cursor | Make test module docstrings and skip messages short and informal; push when coding wrap-up is done | Docstrings/skip text only; re-ran `pytest -m unit` | Good. | Tone in comments should match what you’d actually write; assertions still carry the real spec. |

**Coding wrap-up:** Repo work for this assessment’s code, tests, and K8s/Compose fixes described above is **done and on GitHub**; what’s left is reflection write-up and video per the brief.
...
| 2026-05-13 | Cursor | App pods stuck **`Init:ImagePullBackOff`** / **`ErrImagePull`**; infra pods healthy | Init used **`busybox:1.36`** (extra Docker Hub pull). Swapped inits to **`redis:7-alpine`**, re-applied manifests, restarted rollouts |
| 2026-05-13 | Cursor | Main containers **`ErrImageNeverPull`** while `docker images` listed `eventhub-main-*`; node **`desktop-control-plane`** | Explained **kind + Docker image store** mismatch; README: enable **containerd image store** or use **kubeadm** cluster; optional `kubectl run imgcheck …` |
| 2026-05-13 | Human + Cursor | Same **`ErrImageNeverPull`** until provisioner change | **Human:** switched Docker Desktop Kubernetes to **kubeadm**. **Cursor:** rebuild + `kubectl apply -k k8s/` — all pods **Running** |
